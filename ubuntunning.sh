...
echo -e "${BLUE}[CONFIG] Configurando privilegios NOPASSWD para $REAL_USER...${RESET}\n"
...
unzip -o -q $TEMP_DIR/Hack.zip -d $TEMP_DIR/fonts
unzip -o -q $TEMP_DIR/JetBrainsMono.zip -d $TEMP_DIR/fonts
...
# Instalación maltego
echo -e "\n${BLUE}[*] Descargando Maltego...${RESET}\n"
...
chmod +x maltego.deb
echo -e "\n${BLUE}[*] Instalando Maltego...${RESET}\n"
sudo apt install -y ./maltego.deb

# Instalación VSCode
echo -e "\n${BLUE}[*] Visual Studio Code...${RESET}\n"
...
if [ -f "vscode.deb" ]; then
    echo -e "\n${BLUE}[*] Instalando Visual Studio Code...${RESET}\n"
    sudo apt install -y ./vscode.deb
else
    echo -e "\n${RED}[ERROR] No se pudo descargar VS Code.${RESET}\n"
fi

#Instalacion MSF
echo -e "${BLUE}[DOWNLOAD] Descargando Metasploit Framework...${RESET}\n"
curl https://raw.githubusercontent.com/rapid7/metasploit-omnibus/master/config/templates/metasploit-framework-wrappers/msfupdate.erb > msfinstall && chmod 755 msfinstall && ./msfinstall

#Personalización del dock
echo -e "${BLUE}[CONFIG] Personalizando el Dock de GNOME para $REAL_USER...${NC}"
...
echo -e "${BLUE}[CONFIG] Remapeando Ctrl+Alt+T a Kitty...${NC}"
...
while true; do
    ...
    case $yn in
        [Ss]* ) 
            ...
            break;;
        [Nn]* ) 
            echo -e "${BLUE}[INFO] Directorio conservado en $TEMP_DIR.${NC}"
            break;;
